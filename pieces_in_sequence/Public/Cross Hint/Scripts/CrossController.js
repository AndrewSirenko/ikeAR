...
// @input Asset.Texture[] textures {"showIf":"advanced"}
...
function initializeMesh() {
    ...
    var localPos = script.getSceneObject().getTransform().getLocalPosition(); // Get the local position of CrossController sceneObject

    var offset = new vec3(localPos.x, localPos.y, 0.0); // Cross animation starts at the position of the CrossControllor sceneObject

    if (count > 1) {
        offset = offset.add(new vec3(w / 2, h / 2, 0.0)); // When there is more than one cross, we start building the cross from the corner so we need to offset from center
    }
...
function CrossGenerator(options) {
    ...
    function buildMesh(builder, texture) {
        if (builder.isValid()) {
            var meshVisual = script.getSceneObject().createComponent("Component.RenderMeshVisual");
