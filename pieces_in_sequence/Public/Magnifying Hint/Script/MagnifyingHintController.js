...
function initialize() {
    if (script.foreground) {
        fgTransform = script.foreground.getComponent("ScreenTransform");
        global.tweenManager.startTween(script.foreground, "transitionin");
    }
    if (script.background) {
        bgTransform = script.background.getComponent("ScreenTransform");
        global.tweenManager.startTween(script.background, "transitionin");
    }

    if (script.magnifying) {
        mgTransform = script.magnifying.getComponent("ScreenTransform");
        global.tweenManager.startTween(script.magnifying, "transitionin");
    }

    if (script.label) {
        label = script.label.getComponent("Component.Text");
        label.text = script.mainTextHint;
        global.tweenManager.startTween(script.label, "transitionin");
    }

    srTransform = script.getSceneObject().getComponent("ScreenTransform");
...
function initializeSizing() {
    ...
    var anchors = new vec4(-halfSize.x, halfSize.x, -halfSize.y, halfSize.y);
...
function getOptimalScale(aspect, minBound, maxBound) {
    ...
    return lerp(minBound, maxBound, 1.0 - t);
}
...
function setTexture(sceneObject, previewTexture) {
    var mainPass = sceneObject.getComponent("Component.Image").mainPass;
